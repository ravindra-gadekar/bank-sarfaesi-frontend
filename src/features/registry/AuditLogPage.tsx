...
export default function AuditLogPage() {
  ...
  useEffect(() => {
    setLoading(true);
    apiClient.get<{ success: boolean; data: { logs: AuditEntry[]; total: number; page: number; limit: number; totalPages: number } }>('/audit-logs', {
      params: { page, limit },
    })
      .then((res) => {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-sand-300 dark:border-dark-border text-left">
                  <th className="px-5 py-3 text-xs font-medium text-ink-tertiary dark:text-dark-text-tertiary uppercase tracking-wide">Time</th>
                  ...
                  <th className="px-5 py-3 text-xs font-medium text-ink-tertiary dark:text-dark-text-tertiary uppercase tracking-wide">Resource</th>
                  <th className="px-5 py-3 text-xs font-medium text-ink-tertiary dark:text-dark-text-tertiary uppercase tracking-wide">Details</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log) => (
                  <tr key={log._id} className="border-b border-sand-200 dark:border-dark-border last:border-0">
                    ...
                    <td className="px-5 py-3 text-ink-secondary dark:text-dark-text-secondary">{log.resource}{log.resourceId ? ` #${log.resourceId.slice(-6)}` : ''}</td>
                    <td className="px-5 py-3 text-ink-tertiary dark:text-dark-text-tertiary text-xs max-w-xs truncate">{log.details || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
